...
$TOPPATH = "/var/www/wordpress/wp-content/themes/twentyeleven-child/";
$NAME = "topology";
...
%bitcount = ( 255 => 8, 254 => 7, 252 => 6, 248 => 5, 240 => 4, 224 => 3, 192 => 2, 128 => 1, 0 => 0 );
...
options:
        ...
        --style            drawing style 1, 2 or 3 (default:2)
...
while ( <$remote> ) 
{
	...
	if($line =~ /[\d\.]+\/([\d\.]+)/)
	{
	    $netmask = $1;
	    $bits = 0;
	    foreach(split /\./, $netmask) { $bits += $bitcount{$_} }
	    $line =~ s|/$netmask|/$bits|;
	}
...
sub resolv {
	...
	if ( $l =~ /\"(.*)\" -> "([^[]*)"(.*)/) {
		...
		#$fromn = $from if ! $fromn;
		#$ton = $to if ! $ton;
		if($fromn) { $fromn =~ s/\.austin\.tx\.us\.mesh// }
		else       { $fromn = $from }
		if($ton) { $ton =~ s/\.austin\.tx\.us\.mesh// }
		else     { $ton = $to }
		$l = "\"$fromn\" -> \"$ton\"$rest\n";
	}
	return $l;
